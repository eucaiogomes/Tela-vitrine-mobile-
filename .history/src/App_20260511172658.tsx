...
const VITRINES: Vitrine[] = [
  ...
  { id: 'v8',  nome: 'Vitrine Vendas',         descricao: 'Capacitação do time comercial',         categoria: 'Corporativo', cor: '#EF4444' },
  { id: 'v9',  nome: 'Vitrine Compliance',     descricao: 'Normas, ética e regulatório',           categoria: 'Corporativo', cor: '#6366F1' },
  { id: 'v10', nome: 'Vitrine Tecnologia',     descricao: 'Stack técnica e ferramentas dev',       categoria: 'Produtos',    cor: '#14B8A6' },
  { id: 'v11', nome: 'Vitrine Soft Skills',    descricao: 'Comunicação, foco e colaboração',       categoria: 'Produtos',    cor: '#F97316' },
  { id: 'v12', nome: 'Vitrine Sustentabilidade', descricao: 'ESG e práticas sustentáveis',         categoria: 'Corporativo', cor: '#22C55E' },
  { id: 'v13', nome: 'Vitrine Financeiro',     descricao: 'Finanças corporativas e controladoria', categoria: 'Corporativo', cor: '#A855F7' },
  { id: 'v14', nome: 'Vitrine Marketing',      descricao: 'Growth, branding e campanhas',          categoria: 'Produtos',    cor: '#F43F5E' },
];
...
const getCategoryForVitrine = (vitrineId: string) => {
  for (const [key, data] of Object.entries(CATEGORY_DATA)) {
    if (key === 'all') continue;
    if ((data as any).vitrines?.includes(vitrineId)) {
      const sub = (data as any).subs?.find((item: any) => item.vitriIds?.includes(vitrineId));
      return { category: key, sub: sub?.id ?? null };
    }
  }
  return { category: 'all', sub: null };
};

const getVitrineIdsByFilter = (category: string, sub: string | null) => {
  if (category === 'all') {
    return CATEGORY_DATA.all.vitrines;
  }
  const data = CATEGORY_DATA[category] as any;
  if (!data) return CATEGORY_DATA.all.vitrines;
  if (!sub) return data.vitrines;
  const subItem = data.subs?.find((item: any) => item.id === sub);
...
const MegaMenu = ({ setActiveTab }: { setActiveTab: (tab: string) => void }) => {
  ...
  return (
    ...
    >
      <div className="max-w-full mx-auto px-6 py-10 w-full">
        <div className="grid gap-12 max-w-6xl mx-auto" style={{ gridTemplateColumns: `repeat(${menuSections.length}, 1fr)` }}>
          {menuSections.map((section, idx) => (
            <div key={idx}>
                  ...
                  {section.subs.map((sub: any) => (
                    <li key={sub.id}>
                      <button
                        ...
                        className="text-sm text-gray-700 hover:text-brand-primary hover:font-semibold transition-colors duration-200 text-left"
                      >
                        {sub.label}
                      </button>
                    </li>
              ...
              {/* Vitrines da categoria */}
              {section.vitrines.length > 0 && (
                <div className="border-t border-gray-200 pt-4">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Vitrines</p>
                  <ul className="space-y-2">
                    {section.vitrines.slice(0, 5).map((vitrine: any) => (
                      <li key={vitrine.id}>
                        <button
                          onClick={() => setActiveTab('Explorar')}
                          className="text-sm text-gray-600 hover:text-brand-primary transition-colors duration-200 text-left whitespace-nowrap"
                        >
                          {vitrine.nome}
                        </button>
                      </li>
    ...
    if (query.length > 1) {
      ...
      const filtered = allItems
        .filter(item => {
          ...
          if (query.includes('treinamento') || query.includes('curso')) {
            return item.type === 'COURSE';
          }
          if (query.includes('trilha')) {
            return item.type === 'TRAIL';
          }
...
const ThemeSwitcher = ({
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => { onToggleLayout(1); setActiveTab('Explorar'); }}
      ...
      <button
        onClick={() => { onToggleLayout(2); setActiveTab('Explorar'); }}
      ...
      <button
        onClick={() => { onToggleLayout(3); setActiveTab('Explorar'); }}
        className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-bold transition-all ${layoutVersion === 3 && activeTab === 'Explorar'
          ? 'border-brand-primary text-brand-primary bg-brand-primary/10'
          : 'border-gray-300 text-gray-400 hover:border-gray-400'
          }`}
